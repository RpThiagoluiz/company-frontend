...
export const InformationSecurity = () => {
  return (
    <main className="min-h-screen w-full overflow-x-hidden  bg-stone-100 ">
      <SEO
        title="Segurança da informação - Code Nova"
        description="Segurança da informação, Code Nova Soluções sites e apps"
        name="description"
        type="terms"
      />
...
const TEXTS: TextInfoProps[] = [
  {
    ...
    paragraph: [
      ...
      'A conscientização sobre privacidade é um aspecto crítico no contexto atual e a Code nova Soluções está sintonizada com essa tendência global. Nosso compromisso com a segurança da informação garante uma gestão robusta da privacidade e fornece requisitos e orientações para estabelecer, usar e melhorar práticas de privacidade',
      'Na Code Nova, vemos a segurança como uma ação, não como uma mercadoria. Testes de penetração independentes anuais fortalecem nossas defesas ao identificar e mitigar potenciais vulnerabilidades. Nossos funcionários são reconhecidos como a primeira linha de defesa, ecoando nosso mantra de segurança de trabalhar juntos para tornar o mundo um lugar mais seguro',
    ],
  },
  {
    title: 'Gerenciamento de Incidentes',
    paragraph:
      'O gerenciamento de incidentes é uma parte integral do framework de segurança da Code Nova. Uma abordagem proativa inclui análise da causa raiz, avaliações de risco e sessões de aprendizado para fortalecer as defesas e prevenir futuros incidentes. O registro e monitoramento, facilitados pelo Gerenciamento de Informações e Eventos de Segurança (SIEM), fornecem processamento de eventos em tempo real e alertas',
  },
  {
    title: 'Gerenciamento de Mudanças',
    paragraph:
      'O Gerenciamento de Mudanças da Code nova Soluções garante a implementação controlada de mudanças tecnológicas, reduzindo o risco de interrupções nos negócios. Um processo meticuloso inclui avaliação de riscos, categorização, fluxos de aprovação, comunicação e métricas',
  },
  {
    title: 'Funcionários da Empresa',
    paragraph: [
      'O compromisso da Code nova Soluções com a segurança começa com nossas pessoas. Nossos valores enfatizam uma cultura orientada para as pessoas e focada no cliente. A segurança é incorporada ao nosso processo de contratação, com revisão cuidadosa de currículos, verificações de antecedentes e verificações adicionais para aqueles que lidam com dados do cliente. Os associados passam por treinamento obrigatório de conscientização de segurança e privacidade ao serem contratados, com atualizações regulares e campanhas de simulação de phishing para reforçar a vigilância',
  ...
  },
  {
    ...
    paragraph:
      'A Code nova Soluções implementa controles de gerenciamento de identidade, autenticação e autorização. Ela utiliza autenticação múltipla e controle de acesso baseado em funções (RBAC) para fornecer identificação e autorização seguras',
  },
  {
    title: 'Privacidade',
    paragraph:
      'A Code nova Soluções prioriza a proteção de dados pessoais e de clientes, e está em conformidade com as regulamentações dos EUA e do Brasil',
  },
  {
    title: 'Resumo',
    paragraph:
      'Na Code Nova, a segurança não é apenas uma prática; é um compromisso incorporado à nossa cultura, processos e tecnologia. Nossa dedicação aos mais altos padrões garante um ambiente seguro para os dados dos clientes, funcionários e para a empresa como um todo. Conforme crescemos, nosso foco em segurança e privacidade permanece firme, com a melhoria contínua no centro de nossa abordagem',
  },
]
